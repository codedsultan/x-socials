/**
 * @file src/models/mongo/User.model.ts
 * @description Mongoose model for User (if using MongoDB for users)
 */

import mongoose, { Schema, Model } from 'mongoose';
import type { IUserMongoDocument } from '../../interfaces/entities/user/mongo';

const UserSchema = new Schema<IUserMongoDocument>(
    {
...
UserSchema.methods.setPassword = async function (password: string) {
    ...
    this.password = password; // Simplified - use bcrypt in production
};

UserSchema.methods.matchPassword = async function (password: string) {
    // Compare password logic here
    return this.password === password; // Simplified
};

export const UserModel = mongoose.model<IUserMongoDocument>('User', UserSchema);